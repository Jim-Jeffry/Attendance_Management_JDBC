...
public class Attendance_JDBC {
    ...
    private static final String DB_PASSWORD = "5272";
    ...
    private void studentPortal(int userId) {
        while (true) {
            ...
            if ("In".equalsIgnoreCase(action)) {
                markAttendance("attendance_in", "check_in_time", userId);
            } else if ("Out".equalsIgnoreCase(action)) {
                markAttendance("attendance_out", "check_out_time", userId);
    ...
    private void markAttendance(String tableName, String columnName, int userId) {
        String checkQuery = "SELECT * FROM " + tableName + " WHERE user_id = ? AND DATE(" + columnName + ") = CURDATE()";
        String insertQuery = "INSERT INTO " + tableName + " (user_id, " + columnName + ") VALUES (?, ?)";
        ...
        try (Connection conn = getConnection();
            ...
            try (ResultSet rs = checkStmt.executeQuery()) {
                if (rs.next()) {
                    System.out.println("You have already checked in/out today.");
                } else {
